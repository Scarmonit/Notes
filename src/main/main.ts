...
import { assetSizes, attachments, installAssetProtocol, openAttachment, pickAttachments, registerAssetScheme, saveAttachment, sweepOrphans } from './attachments';
import { destroyCapture, hideCapture, showCapture, toggleCapture } from './capture';
import { installContextMenu, setContextAttachment } from './context-menu';
...
if (squirrelLaunch) {
  ...
  ipcMain.handle(IPC.openAttachment, (_event, name: string) => openAttachment(String(name)));
  ipcMain.handle(IPC.assetSizes, (_event, names: string[]) => assetSizes(Array.isArray(names) ? names : []));
  ipcMain.on(IPC.contextAttachment, (_event, name: string | null) => setContextAttachment(typeof name === 'string' ? name : null));
