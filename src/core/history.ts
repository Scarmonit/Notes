...
export interface History {
  readonly dir: string;
  keepNow(note: Note): Promise<void>;
  record(notes: Note[], alsoKeep: ReadonlySet<string>, options?: { sweep?: boolean }): Promise<void>;
  forgetHistory(id: string): Promise<void>;
  listHistory(id: string): Promise<SnapshotSummary[]>;
  getSnapshot(id: string, at: number): Promise<Snapshot | null>;
}

export function createHistory(root: string): History {
  const dir = pathsFor(root).history;
  ...
  function fileFor(id: string): string {
    return path.join(dir, `${encodeURIComponent(id).slice(0, 120)}.json`);
  }
  ...
  function keepNow(note: Note): Promise<void> {
    return serial(async () => {
   ...
   * stay — the ones waiting in the trash, which may yet come back. The sweep
   * that throws the rest away runs once per process, and only when asked: a
   * caller that has not read the trash must not sweep on its behalf.
   */
  function record(notes: Note[], alsoKeep: ReadonlySet<string>, options: { sweep?: boolean } = {}): Promise<void> {
    return serial(async () => {
      const now = Date.now();
  ...
  async function forgetHistory(id: string): Promise<void> {
    await fs.unlink(fileFor(id)).catch(() => undefined);
    lastKnown.delete(id);
  }

  /** Every kept version of one note, newest first, without the bodies. */
  async function listHistory(id: string): Promise<SnapshotSummary[]> {
    const snapshots = await readFileFor(id);
    return snapshots.map(summarize).reverse();
  }

  /** One kept version in full, by the moment it was taken. */
  async function getSnapshot(id: string, at: number): Promise<Snapshot | null> {
    const snapshots = await readFileFor(id);
    return snapshots.find((s) => s.at === at) ?? null;
  }

  return { dir, keepNow, record, forgetHistory, listHistory, getSnapshot };
}
