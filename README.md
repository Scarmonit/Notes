...
- Attach images by pasting, dropping a file anywhere on the window, or `Ctrl+Shift+I`; they render as pictures inline, resize by their corner handle, and live in the notes folder's `attachments` (any other kind of file attaches too — see below)
...
- **The Layout sheet is set like the page it lays out**: a ruled pad, the section names in a margin column against a rule, and at its head a proof of the window drawn from the same tokens the page is set from. Drag Line width and the column widens; drag Reading size and the lines thicken and fewer fit; drag Interface size and only the sidebar and header grow; untick Margin or Outline and they leave the proof as they leave the page (Layout, `Ctrl+,`)
- Two text sizes, kept apart: **Interface size** scales everything that is not the note — the list, the menus, the sheets — up to 140 %, and **Reading size** scales the note itself from 85 % to 160 % (Layout, `Ctrl+,`; `notes ui set uiScale 1.2`, `notes ui set readingScale 1.1`)
...
- **Folders**: real nested directories inside the notes folder, so a note has somewhere to live rather than only something to be tagged. The sidebar browses them above the note list — **All notes** first, then the tree, a folder standing for everything beneath it — and the pane header says quietly which folder the note on screen is in. `Ctrl+Alt+M` files a note in another folder (a fuzzy picker of whole paths, which can make one on the way); `Ctrl+Alt+F` goes to the tree; New folder…, Rename, Move and Delete are in the Note menu. A folder is exclusive — one note, one place — while `#tags` stay non-exclusive: folders say *where a note lives*, tags say *what is true about it*. Two notes in different folders may share a title, so a bare `[[Plan]]` that names both is flagged rather than guessed and `[[Work/Plan]]` says which. Nothing moves when you upgrade: the root is a permanent place, not an inbox to empty. `folder:Work` searches a folder and everything beneath it, `folder:/` the notes at the root
- **Peek a link**: resting on a `[[link]]`, a backlink, a related note or a row in the list shows what it points at, in a small card beside it — and shows *exactly* what the link named, so `[[Plan#^k3n9dq]]` peeks that block and `[[Plan#A heading]]` that section. A name two notes answer to shows both, with the folder that tells them apart. It changes nothing at all: no tab, no pane, no history, no caret, no scroll — which is the whole point of it, and why the card is deliberately lighter than the preview (no diagrams, no highlighting, no embeds expanded, at most the first 12 KB of a long note). `Alt+P` does the same from the keyboard and pins the card; **Preview on hover** in Layout turns the pointer half off without touching the command
- **Slash commands**: typing `/` at the start of a line, or after a space, opens the commands that put something *here* — task, date, divider, template, table, attachment, block link — each with its chord printed beside it. It is not a second palette: the palette answers "what can Notes do?", a slash answers "what can Notes insert here?". The `/` and whatever is typed after it stay ordinary characters in the note until a command actually runs, so `and/or` and a path are never swallowed, `Esc` leaves what you typed where it is, and one `Ctrl+Z` undoes the query and the insertion together
- **Right-click a note**: the commands that are about *that* note, on the row itself — pin, rename, properties, file it in a folder, take it out of one, export, merge, show it in Explorer, delete — each with its chord printed beside it. It acts on the note you clicked and leaves the one you are reading on screen, so a note can be filed or deleted without first having to go and open it, and the row stays marked while the menu is up so you can see which note is about to change. **Take out of folder** is new and puts a note back at the top level in one click; it is greyed rather than hidden on a note already there. The menu is a view of the same registry as everything else, so a command cannot drift out of it. Right-clicking a word in the note itself still gives you the spellchecker’s own menu
- **Named workspaces**: save which notes are open in which panes under a name, and switch between arrangements — the release one, the writing one. A workspace holds the panes and their tabs and nothing else: the folder you were browsing, the search box and how you read all stay exactly as they were, so switching is spatial rather than a change of mode. Snapshots never quietly update themselves, and a note that has been deleted is left out rather than refusing the whole arrangement. **Workspaces…** in the Window menu
- **Today's note**: `Ctrl+Alt+D` opens the dated note for today, making it if it is not written yet, and **Journal for a day…** takes `yesterday`, `friday`, `+3d` or `2026-09-01`. Where an entry goes is one setting (Layout, `Ctrl+,` → Journal), a path format where a slash makes a folder — `Journal/YYYY/YYYY-MM-DD` by default, so a year stays browsable and five years do not become one list. Nothing marks a note as an entry: **the note occupying that path for that date is that date's note**, so an entry already there is opened and never written to, and changing the format moves nothing that is already written. A `#template` note can start each day, expanded against *that day* rather than the moment you happened to type it. `notes journal [date]`
- **Properties**: the front-matter keys the app does not own — a `status: draft` written in Obsidian used to be carried through invisibly, and is now something you can see, search and change. **Properties…** in the Note menu (`Ctrl+Shift+A`, which also edits the other names a note answers to) shows every key the note carries, with completion from the words the notebook already uses so a second spelling is a choice rather than an accident; **All properties…** lists the whole vocabulary and what each key is worth. `prop:status` finds notes carrying a key and `prop:status=draft` those holding a value, in the search box and on the command line alike. **Nothing is ever added that you did not ask for**, and one changed key rewrites one line: comments, nested YAML and a key written twice all come through exactly as they were. `notes props`, `notes props set`, `notes props remove`
- **Block references**: `[[Note#Heading]]` has always worked; now `[[Note#^k3n9dq]]` points at one paragraph, list item, quote, table or code block, and `![[Note#^k3n9dq]]` embeds it live. **Copy a link to this block** gives the block at the caret an address and puts the link on the clipboard; **Link to a block…** picks a note, then a block in it, and writes the link here. `[[#^k3n9dq]]` and `[[#Heading]]` point inside the note they are written in. An address is minted **only when you ask for one** — nothing appears in your markdown from opening, rendering or saving a note — and once written it is ordinary text you can delete. Moving addressed text to another note says what it will break before it does it. `notes show <note> --block ^k3n9dq`
- **Callouts**: `> [!info] A title` makes a blockquote a box that says what it is, in [Obsidian's syntax](https://obsidian.md/help/callouts) — the thirteen kinds and their aliases (note, tip, warning, danger, quote…), an unknown kind wearing its own name. Every kind is the same quiet box with a different word on it; the app has one accent and it is spent. `[!note]-` starts folded and `[!note]+` open, as a real `<details>` in the preview and the HTML export, and shown open on paper. In the editor the whole box takes a rule and a raised ground while the characters stay yours. **Insert callout** (`Ctrl+Alt+C`, or `/callout`) puts one under the caret
- **Footnotes beside your writing**: `[^1]` refers, `[^1]: words` defines (four spaces continue it), and `^[words]` writes a footnote where it stands. They are numbered by first reference, gathered at the end of the preview and every export with a way back to each reference, and a superscript peeks its words on hover. The rail beside the note — under the outline — lists them: a number goes to the reference nearest where you were, the words open right there to edit (`Ctrl+Enter` commits, `Esc` cancels), a definition nobody refers to and a reference nobody defines are both named, and **↩ Back** puts you where you were writing. **Insert footnote** (`Ctrl+Alt+E`, or `/footnote`) writes the next number here and its line at the end; **Footnotes** in the View menu (`Ctrl+Alt+O`) hides the rail
- **Fold headings and lists**: a chevron in the gutter beside a heading, or a list item with lines under it, folds what it owns down to that line — `…` says something is hidden, a click or `Ctrl+Alt+.` opens it again, `Ctrl+Alt+[` folds everything and `Ctrl+Alt+]` unfolds it. Folding never touches the file: the lines are still the text, only not on screen, and which are folded is remembered per note by the window. An edit that would cross a fold opens it first; a search hit, an outline click or a footnote jump into one opens it on the way. The preview and every export show the whole note
- **Tables and cards for saved searches**: a saved search can open as a table — one row per note the search finds, its front-matter properties as columns — or as cards, in the place of a note, as a tab beside the others (as [Obsidian's Bases](https://obsidian.md/help/bases)). **Layout for this search…** (`Ctrl+Alt+V`) chooses List, Table or Cards for the search in the box or the table in front of you; **Columns…** picks and orders the properties, a header click sorts (numbers as numbers, missing values last), **Group by…** gathers the notes under each value. A cell is edited in place — a checkbox for a yes/no, a number field for a number, `[a, b]` for a list — through the same one-line write as Properties, and like Properties it is not undoable. Nothing new reaches the disk: the query is still the query, the layout sits beside it in the saved search, and the command line reads the search as it always did
- **Attach any file**: a PDF, a recording, a spreadsheet, anything up to 50 MB, by pasting, dropping or `Ctrl+Shift+I`, kept beside the pictures under a random name with the file's own name as the link's words. A PDF alone on its line is shown in a frame, audio and video get a player, and everything else is a chip with the name and size that opens in whatever Windows opens it with (**Open attachment**, `Ctrl+Alt+A`, or the right-click menu) — a program or a script attaches but is never launched from here. Markdown export copies every attachment beside the file; the HTML export keeps pictures inside the page and the rest in a folder beside it; a PDF or PNG names what it cannot show (as [Joplin's attachments](https://joplinapp.org/help/apps/attachments/))
...
- Notes are a tree of markdown files, `%APPDATA%\Notes\notes\<folder>\<title>.md`, each with a small front-matter block (id, dates, pinned). A note’s folder is where its file is and nothing else — it is never written into the file — so a note moved in Explorer, by OneDrive or by `git pull` arrives as a note that moved, keeping its id, its links and its history. Put the folder in OneDrive, Dropbox or git and they are backed up and readable anywhere; files changed or added by anything else show up in the app while it runs. Autosaved (atomic writes, flushed on close), with the snapshots beside them in `history` and deleted notes in `trash`. The first launch after 0.11 moves the old `notes.json` into the folder and keeps it as `notes.json.migrated`
...
| `Ctrl+Alt+M` | File this note in another folder |
| `Ctrl+Alt+F` | Go to the folder tree |
| `Ctrl+\` | Toggle sidebar |
...
| `notes move <note> <folder>` | file a note in another folder, keeping its name, its links and its history |
| `notes folders list` (ls) | every folder, empty ones included, with how many notes are in each |
| `notes folders new <path>` (add) | make a folder, and every folder above it |
| `notes folders rename <folder> <name>` | change a folder's own name, leaving it where it is |
| `notes folders move <folder> <destination>` | put a folder, and everything in it, inside another |
| `notes folders delete <folder>` (rm) | remove a folder that holds nothing — no folder command ever takes a note with it |
| `notes journal [date]` | open or make the dated note for a day; today's when no day is given |
| `notes props set <note> <key> [value]` | set one property, or replace it |
| `notes props remove <note> <key>` (rm) | take one property off a note |
