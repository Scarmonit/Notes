...
- **Peek a link**: resting on a `[[link]]`, a backlink, a related note or a row in the list shows what it points at, in a small card beside it — and shows *exactly* what the link named, so `[[Plan#^k3n9dq]]` peeks that block and `[[Plan#A heading]]` that section. A name two notes answer to shows both, with the folder that tells them apart. It changes nothing at all: no tab, no pane, no history, no caret, no scroll — which is the whole point of it, and why the card is deliberately lighter than the preview (no diagrams, no highlighting, no embeds expanded, at most the first 12 KB of a long note). `Alt+P` does the same from the keyboard and pins the card; **Preview notes on hover** in Layout turns the pointer half off without touching the command
